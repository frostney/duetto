...
```bash
curl -fsSLO https://github.com/frostney/lwpt/releases/download/0.5.1/lwpt-0.5.1-macos-arm64.tar.gz
curl -fsSLO https://github.com/frostney/lwpt/releases/download/0.5.1/lwpt-0.5.1-checksums.txt
shasum -a 256 -c <(grep macos-arm64 lwpt-0.5.1-checksums.txt)
tar xzf lwpt-0.5.1-macos-arm64.tar.gz
export PATH="$PWD/lwpt-0.5.1-macos-arm64:$PATH"   # or copy lwpt into ~/bin
```
...
`frostney/lwpt` 0.5.1 release tag with include filters (see `lwpt.toml`);
